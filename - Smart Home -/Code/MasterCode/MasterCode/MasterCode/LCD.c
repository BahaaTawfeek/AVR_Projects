...
void LCD_vInit(void)
{
	...
	DIO_vsetPinDir('A',4,1);						  // make the direction of pin4 as output port
	DIO_vsetPinDir('A',5,1);						  // make the direction of pin5 as output port	
	DIO_vsetPinDir('A',6,1);						  // make the direction of pin6 as output port	
	DIO_vsetPinDir('A',7,1);						  // make the direction of pin7 as output port		
	
	DIO_vsetPinDir('B',EN,1);					      // make the direction of pin0 (EN)  as output port
	DIO_vsetPinDir('B',RW,1);						  // make the direction of pin1 (RW)  as output port
	DIO_vsetPinDir('B',RS,1);						 // make the direction of pin2 (RS)  as output port
	
	DIO_vwritePin('B',RW,0);					     //write mode select
...
void LCD_vsend_cmd(char cmd)
{
	...
	DIO_vwrite_high_nibble('A',cmd>>4);			//  shifted right to write high nibble of cmd  
	DIO_vwritePin('B',RS,0);			//  Make RS pin low, RS = 0 (command reg.)
	vsend_falling_edge();				// enable LCD display
	DIO_vwrite_high_nibble('A',cmd);			//   write high nibble of cmd (low nibble )
	DIO_vwritePin('B',RS,0);			//  Make RS pin low, RS = 0 (command reg.)
	vsend_falling_edge();				// enable LCD display
...
void LCD_vsend_char(char data)
{
	...
	DIO_vwrite_high_nibble('A',data>>4);			//  shifted right to write high nibble of data
	DIO_vwritePin('B',RS,1);			//  Make RS pin high, RS = 1 (Data reg.)
	vsend_falling_edge();				// enable LCD display
	DIO_vwrite_high_nibble('A',data);			//   write high nibble of cmd (low nibble )
	DIO_vwritePin('B',RS,1);			//  Make RS pin high, RS = 1 (data reg.)
	vsend_falling_edge();				// enable LCD display
